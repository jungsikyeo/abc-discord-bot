...
from typing import Union, Optional, Awaitable, Callable, List
...
class Paginator:
    ...
    """
    def __init__(self, bot: Union[Client, commands.Bot], pages: Optional[List[Page]] = None):
        self.bot = bot
    ...
    async def send(self, channel: Union[TextChannel, DMChannel], pages: Optional[List[Page]] = None, type: int = 2, timeout: Optional[int] = 60, author: Optional[Union[discord.Member, discord.User]] = None, disable_on_timeout: bool = True, timeout_callback: Optional[Callable[[], Awaitable[None]]] = None):
        """
        ...
        if type == 1:
            ...
            self.msg = await channel.send(content=pages[0].content, embed=embed, view=view)

            current_page = 0

            while not self.bot.is_closed():
                try:
                    interaction = await self.bot.wait_for('interaction', check=lambda i: i.message.id == self.msg.id and (i.user == author if author is not None else i.user != self.bot.user), timeout=timeout)
                    ...
                    if disable_on_timeout:
                        ...
                        await self.msg.edit(content=pages[current_page].content, embed=pages[current_page].embed, view=view)
                    if timeout_callback:
                        await timeout_callback()
                    break
